...
def cleave_ub(hgnc_id: str, me_input_model, ub_args: Dict[str, Any], compress_mrna: bool, stochastic: bool, seeds: List[int]):
    ...
    translation_elongation_c, unfolded_protein_c = build_protein.c_trln.translate_protein_cytosolic(gene_info, me_input_model,
                                                                                                    mrna_transcript_c,
                                                                                                    mrna_deg_proxy)
    ...
    processed_unfolded_protein_c = Protein(id_='processed_unfolded', compartment='c', me_input_model=me_input_model,
                                           gene_info=gene_info)
    ...
    dcp = degradation.degrade(folded_protein_c, **{'me_input_model': me_input_model, 'ub_args': ub_args})
...
def build_ribosome_protein_expression_reactions(me_input_model, ub_args: Dict[str, Any], compress_mrna: bool, stochastic: bool, seeds: List[int]):
    ...
    for i in rs_ids:
        ...
        mrna_expression_reactions, mrna_transcript_c, mrna_deg_proxy = build_mrna.get_mrna_expression_reactions(
            gene_info=gene_info, me_input_model=me_input_model, compress_mrna=compress_mrna)
        protein_expression_reactions, protein_metabolites = build_protein.get_protein_expression_reactions(gene_info, me_input_model,
                                                                                                           mrna_transcript_c,
                                                                                                           mrna_deg_proxy,
                                                                                                           ub_args)
    ...
    # ubiquitin fusions--------------------------------------------------------------------------------------
    # RPS27A
    to_add, folded_protein_c, folded_protein_n = cleave_ub(hgnc_id=RPS27A_HGNC, me_input_model = me_input_model, ub_args=ub_args,
                                                           compress_mrna=compress_mrna, stochastic=stochastic,
                                                           seeds=seeds[seed_idx: seed_idx + 2])
    seed_idx += 2
    rs_expression_reactions += to_add
    rs_protein_metabolites += [folded_protein_c, folded_protein_n]
    # RPL40
    to_add, folded_protein_c, folded_protein_n = cleave_ub(hgnc_id=RPL40_HGNC, me_input_model = me_input_model, ub_args=ub_args,
                                                           compress_mrna=compress_mrna, stochastic=stochastic,
                                                           seeds=seeds[seed_idx: seed_idx + 2])
...
def build_rrna5s_reactions(rpl5_n, rpl11_n, me_input_model):
    # TRANSCRIPTION - basically emulates Transcript.transcript_elongation reaction
    pre_rrna5s_n = rRNA(me_input_model=me_input_model, metabolite_name='pre_5s', seq=pre_rrna_5s_seq, compartment='n')
    ...
    rrna5s_n = rRNA(me_input_model=me_input_model, metabolite_name='5s', seq=rrna_5s_seq, compartment='n')
...
def build_other_rrna_reactions(rrna5s_complex_n, rs_protein_metabolites, rl_protein_metabolites,
                               rpl5_n, rpl11_n, rrna5s_c, me_input_model, reversible_complex_formation=False):
    # https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6315592/ figure 2

    # 47s transcription------------------------------------------------------------------------------------
    rrna_47s_n = rRNA(me_input_model=me_input_model, metabolite_name='47s', seq=rrna_47s_seq, compartment='n')
    ...
    ets_5_frag1_n = RNA_fragment(me_input_model=me_input_model, metabolite_name='5_frag1', seq=ets_5_frag1_seq, compartment='n', fragment_type='ets')  # ets5
    ets_3_n = RNA_fragment(me_input_model=me_input_model, metabolite_name='3', seq=ets_3_seq, compartment='n', triphosphate=False, fragment_type='ets')
    rrna_45s_n = rRNA(me_input_model=me_input_model, metabolite_name='45s', seq=rrna_45s_seq, compartment='n', triphosphate=False)
    ...
    rrna_30s_n = rRNA(me_input_model=me_input_model, metabolite_name='30s', seq=rrna_30s_seq, compartment='n', triphosphate=False)
    rrna_32_5s_n = rRNA(me_input_model=me_input_model, metabolite_name='32_5s', seq=rrna_32_5s_seq, compartment='n', triphosphate=False)
    ...
    rrna_26s_n = rRNA(me_input_model=me_input_model, metabolite_name='26s', seq=rrna_26s_seq, compartment='n', triphosphate=False)
    ets_5_frag3_n = RNA_fragment(me_input_model=me_input_model, metabolite_name='5_frag3', seq=ets_5_frag3_seq, compartment='n', triphosphate=False,
                                 fragment_type='ets')
    ...
    rrna_21s_n = rRNA(me_input_model=me_input_model, metabolite_name='21s', seq=rrna_21s_seq, compartment='n', triphosphate=False)
    ets_5_frag4_n = RNA_fragment(me_input_model=me_input_model, metabolite_name='5_frag4', ets_5_frag4_seq, fragment_type='ets', compartment='n',
                                 triphosphate=False)
    ...
    rrna_21sc_n = rRNA(me_input_model=me_input_model, metabolite_name='21sc', seq=rrna_21sc_seq, compartment='n', triphosphate=False)
    ...
    rrna_18se_n = rRNA(me_input_model=me_input_model, metabolite_name='18se', seq=rrna_18se_seq, compartment='n', triphosphate=False)
    its_1_frag1_n = RNA_fragment(me_input_model=me_input_model, metabolite_name='1_frag1', seq=its_1_frag1_seq, fragment_type='its', compartment='n',
                                 triphosphate=False)
    ...
    rrna_18se_processed_n = rRNA(me_input_model=me_input_model, metabolite_name='18se_processed', seq=rrna_18se_processed_seq, compartment='n', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
    ...
    rxn[metab.atp_n], rxn[metab.ntp_map_n['U']], rxn[metab.ppi_n] = -int(0.125 * e_index), -int(0.125 * e_index), int(
        0.25 * e_index) - 1
    ...
    # gtp hydrolysis
    rxn[metab.ntp_map_c['G']], rxn[metab.h2o_c], rxn[metab.ndp_map_c['G']], rxn[metab.pi_c], rxn[
        metab.h_c] = -1, -1, 1, 1, 1
    ...
    rrna_18s_c = rRNA(me_input_model=me_input_model, metabolite_name='18s', seq=rrna_18s_seq, compartment='c', triphosphate=False)
    its_1_frag2_c = RNA_fragment(me_input_model=me_input_model, metabolite_name='1_frag2', seq=its_1_frag2_seq, fragment_type='its', compartment='c',
                                 triphosphate=False)
    ...
    rrna_32s_n = rRNA(me_input_model=me_input_model, metabolite_name='32s', seq=rrna_32s_seq, compartment='n', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
    ...
    rrna_12s_n = rRNA(me_input_model=me_input_model, metabolite_name='12s', seq=rrna_12s_seq, compartment='n', triphosphate=False)
    rrna_28_5s_n = rRNA(me_input_model=me_input_model, metabolite_name='28_5s', seq=rrna_28_5s_seq, compartment='n', triphosphate=False)
    ...
    rrna_28s_n = rRNA(me_input_model=me_input_model, metabolite_name='28s', seq=rrna_28s_seq, compartment='n', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
    ...
    rrna_7s_n = rRNA(me_input_model=me_input_model, metabolite_name='7s', seq=rrna_7s_seq, compartment='n', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
    ...
    rrna_5_8s_plus_40_n = rRNA(me_input_model=me_input_model, metabolite_name='5_8s_plus_40', seq=rrna_5_8s_plus_40_seq, compartment='n', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
    ...
    rrna_6s_n = rRNA(me_input_model=me_input_model, metabolite_name='6s', seq=rrna_6s_seq, compartment='n', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
    ...
    rrna_5_8s_c = rRNA(me_input_model=me_input_model, metabolite_name='5_8s', seq=rrna_5_8s_seq, compartment='c', triphosphate=False)
    base_counts_deg, elements_deg = func.get_base_counts_and_elements(deg_seq)
...
def build_ribosome(me_input_model, ub_args: Dict[str, Any], compress_mrna: bool, reversible_complex_formation: bool, stochastic: bool, seed: int):
    ...
    with func.HiddenPrints():
        ...
        rs_expression_reactions, rs_protein_metabolites, rl_expression_reactions, rl_protein_metabolites = build_ribosome_protein_expression_reactions(
            me_input_model = me_input_model, ub_args=ub_args, compress_mrna=compress_mrna,
    ...
    rrna5s_reactions, rrna5s_complex_n, rrna5s_c = build_rrna5s_reactions(rpl5_n, rpl11_n, me_input_model)
    other_rrna_reactions, mature_ribosomal_precomplexes, mature_rrna_metabolites = build_other_rrna_reactions(
        ...
        rpl5_n, rpl11_n, rrna5s_c, me_input_model=me_input_model,
