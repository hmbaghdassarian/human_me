...
def cleave_ub(hgnc_id: str, model_metabolites, ub_args: Dict[str, Any], compress_mrna: bool, stochastic: bool, seeds: List[int]):
...
def build_ribosome_protein_expression_reactions(model_metabolites, modified_trna_transcript_c, charged_trna_map,
    ...
    for i in rl_ids:
        ...
        protein_expression_reactions, protein_metabolites = build_protein.get_protein_expression_reactions(gene_info, 
                                                                                                           mrna_transcript_c,
                                                                                                           mrna_deg_proxy,
                                                                                                           ub_args, model_metabolites=model_metabolites)
        protein_expression_reactions = [r for r in protein_expression_reactions if
                                        r.id.split('_')[-1] not in exclude]  # no nuclear degradation
    ...
    # ubiquitin fusions--------------------------------------------------------------------------------------
    # RPS27A
    to_add, folded_protein_c, folded_protein_n = cleave_ub(hgnc_id=RPS27A_HGNC, model_metabolites=model_metabolites, ub_args=ub_args,
    ...
    # RPL40
    to_add, folded_protein_c, folded_protein_n = cleave_ub(hgnc_id=RPL40_HGNC,  model_metabolites=model_metabolites, ub_args=ub_args,
                                                           compress_mrna=compress_mrna, stochastic=stochastic,
                                                           seeds=seeds[seed_idx: seed_idx + 2])
...
def build_other_rrna_reactions(rrna5s_complex_n, rs_protein_metabolites, rl_protein_metabolites,
    ...
    ets_5_frag4_n = RNA_fragment(model_metabolites=model_metabolites, metabolite_name='5_frag4', ets_5_frag4_seq, fragment_type='ets', compartment='n',
                                 triphosphate=False)
...
def build_ribosome(model_metabolites, modified_trna_transcript_c, charged_trna_map, ub_args: Dict[str, Any], compress_mrna: bool, reversible_complex_formation: bool, stochastic: bool, seed: int):
    ...
    with func.HiddenPrints():
        ...
        rs_expression_reactions, rs_protein_metabolites, rl_expression_reactions, rl_protein_metabolites = build_ribosome_protein_expression_reactions(
            model_metabolites=model_metabolites, modified_trna_transcript_c=modified_trna_transcript_c, charged_trna_map=charged_trna_map,
