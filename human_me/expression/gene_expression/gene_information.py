...
class GeneInformation:
    ...
    def __init__(self, hgnc_id, premrna_seq, mrna_seq, protein_seq,
                 ...
                 ptms: dict = None, tmd=0, sp=False, polyA_length=None, n_exons=None,
        ...
        if type(self.coupling_params['ptr']) == float or type(self.coupling_params['ptr']) == int:  # specified ptr
            ...
            if self.coupling_params['ptr'] not in params.ptr.columns.tolist():
                ...
                if not pd.isna(val):  # the if statement above can still product a nan
                    ...
                    self.coupling_params['ptr'] = params.ptr.groupby(params.ptr.HGNC_ID).median().loc[:,
                                                  self.coupling_params['ptr']].median()
        if self.coupling_params['ptr'] is None or pd.isna(self.coupling_params['ptr']):  # totally unspecified ptr
            if self.hgnc_id in params.ptr.HGNC_ID.tolist():
                self.coupling_params['ptr'] = params.ptr.groupby(params.ptr.HGNC_ID).median().loc[self.hgnc_id,
                                              :].median()
            else:
    ...
    def get_final_locations(self, reactions=None, nonmachinery_locations: list = None):
    ...
    def check(self):
        ...
        if len(self.ptms) > 0:
            ...
            if len(set(self.ptms.keys()).difference(params.allowed_ptms.keys())) > 0:
                ...
                self.ptms = {k: v for k, v in self.ptms.items() if k in params.allowed_ptms.keys()}
...
def generate_from_psim(hgnc_id, psim=params.psim_me, machinery_list=mach.metabolic_machinery,
                       reactions=None, nonmachinery_locations: list = None, stochastic=False, seed=None):
