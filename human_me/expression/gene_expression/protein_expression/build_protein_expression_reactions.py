...
def fold_protein_cytosolic(gene_info, unfolded_protein_c: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    if gene_info.L_protein > 100:  # chaperone assisted for larger proteins - https://www.nature.com/articles/nature10317
        rxn = func.hydrolyze_atp(rxn, n_atp=gene_info.L_protein * params.PROTEOLYSIS_TRANSLOCATION_ATP_COST, compartment='c', model_metabolites=model_metabolites)
...
# # Nuclear Reactions
def transport_nuclear_protein(gene_info, folded_protein_c: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    if folded_protein_c.formula_weight / 1000 > params.NUCLEAR_DIFFUSION_LIMIT:
        ...
        import_rxn[model_metabolites.gtp_n], import_rxn[model_metabolites.h2o_n], import_rxn[model_metabolites.gdp_n], import_rxn[model_metabolites.pi_n], import_rxn[model_metabolites.h_n] = -1, -1, 1, 1, 1
...
def get_nuclear_reactions(gene_info, folded_protein_c: Protein, ub_args: Dict[str, Any], model_metabolites) -> ProteinExpressionType:
    ...
    """
    nuclear_import, folded_protein_n = transport_nuclear_protein(gene_info, folded_protein_c, model_metabolites)
    nuclear_degradation_reactions = degradation.degrade(macromolecule=folded_protein_n, model_metabolites=model_metabolites, **{'ub_args': ub_args})
...
def transport_mitochondrial_matrix(gene_info, unfolded_protein_c: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    rxn = func.hydrolyze_atp(rxn, n_atp=gene_info.L_protein * params.TRANSPORT_TRANSLOCATION_ATP_COST, compartment='m', model_metabolites=model_metabolites)
...
def get_mitochondrial_reactions(gene_info, unfolded_protein_c: Protein, compartments: List[str], model_metabolites) -> Tuple[List[ProteinExpressionType]]:
    ...
    """
    mitochondrial_matrix_transport, pre_protein_m = transport_mitochondrial_matrix(gene_info, unfolded_protein_c, model_metabolites=model_metabolites)
    ...
    if 'm' in compartments:
        ...
        mitochondrial_reactions += degradation.degrade(macromolecule=processed_protein_m, model_metabolites=model_metabolites)
        for r in mitochondrial_reactions:
            r._final_compartments.add('m')
    ...
    if 'i' in compartments:
        ...
        mitochondrial_reactions += [mitochondrial_inter_transport] + degradation.degrade(macromolecule=processed_protein_i, model_metabolites=model_metabolites)
        for r in mitochondrial_reactions:
...
def transport_peroxisome(gene_info, folded_protein_c: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    rxn = func.hydrolyze_atp(rxn, n_atp=(gene_info.L_protein + mach.L_PEX5) * params.TRANSPORT_TRANSLOCATION_ATP_COST,
                             compartment='x', model_metabolites=model_metabolites)
...
def get_peroxisomal_reactions(gene_info, folded_protein_c: Protein, model_metabolites):
    ...
    pr = [peroxisomal_transport] + degradation.degrade(macromolecule=folded_protein_x, model_metabolites=model_metabolites)
...
def post_translational_translocation(gene_info, unfolded_protein_c: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    rxn = func.hydrolyze_atp(rxn, n_atp=1, compartment='c', model_metabolites=model_metabolites)
    ...
    if gene_info.tmd > 0 or 'pm' in gene_info.all_locations:  # membrane secreted protein
        ...
        rxn = func.hydrolyze_atp(rxn, n_atp=number_BiP, compartment='r', model_metabolites=model_metabolites)
        post_translational_translocation_r.add_metabolites(rxn)
        ptt_reactions += [post_translational_translocation_r]
...
def co_translational_translocation(gene_info, mrna_transcript_c, mrna_deg_proxy, modified_trna_transcript_c, charged_trna_map, model_metabolites):
    if gene_info.L_protein <= params.PTT_LENGTH:
    ...
    unprocessed_protein_r = Protein(compartment='r', id_='unprocessed_folded', model_metabolites=model_metabolites, gene_info=gene_info)
    ...
    folded_protein_r = Protein(compartment='r', id_='folded', model_metabolites=model_metabolites, gene_info=gene_info)

    rxn = {model_metabolites.seq_amino_acid_map_r[aa]: count for aa, count in sp_aa_counts.items()}
    rxn[model_metabolites.h2o_r] = -params.L_SP
...
def form_disulfide_bond(gene_info, folded_protein_r: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    rxn = {folded_protein_r: -1, modified_protein_dsb_r: 1, model_metabolites.o2_r: -number_DSB, model_metabolites.h2o2_r: number_DSB}
...
def form_gpi(gene_info, modified_protein_r: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    rxn = dict()
#     rxn[model_metabolites.hdca_r], rxn[model_metabolites.gpi_hs_r], rxn[model_metabolites.h_r], rxn[model_metabolites.h2o_r], rxn[model_metabolites.gpi_sig_r] = 1,-1,1,-1,1
    rxn[model_metabolites.hdca_r], rxn[model_metabolites.gpi_hs_r], rxn[model_metabolites.h_r], rxn[model_metabolites.h2o_r] = 1, -1, 1, -1
...
def modify_protein_er(gene_info, folded_protein_r: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    if 'dsb' in gene_info.ptms and gene_info.ptms['dsb'] > 0:
        disulfide_bond_formation, modified_protein_r = form_disulfide_bond(gene_info, folded_protein_r, model_metabolites)
    ...
    if 'gpi' in gene_info.ptms and gene_info.ptms['gpi'] > 0:  # == 1 but doesn't matter bc of gene_info checks
        ...
        gpi_formation, modified_protein_r = form_gpi(gene_info, modified_protein_r, model_metabolites)
...
def import_golgi(gene_info, modified_protein_r: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    # gtp hydrolysis
    rxn[model_metabolites.ntp_map_c['G']], rxn[model_metabolites.h2o_c], rxn[model_metabolites.ndp_map_c['G']], rxn[model_metabolites.pi_c], rxn[model_metabolites.h_c] = -94, -94, 94, 94, 94
...
def glycosylate_o_linked(gene_info, protein_g: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    rxn = {protein_g: -1, modified_protein_og_g: 1, model_metabolites.udpacgal_g: -number_Oglycans,
           model_metabolites.udpgal_g: -number_Oglycans, model_metabolites.uacgam_g: -number_Oglycans, model_metabolites.h_g: 3 * number_Oglycans,
           model_metabolites.udp_g: 3 * number_Oglycans}
...
def modify_protein_golgi(gene_info, protein_g: Protein, model_metabolites) -> ProteinExpressionType:
    ...
    if 'og' in gene_info.ptms and gene_info.ptms['og'] > 0:
        o_glycosylation, modified_protein_g = glycosylate_o_linked(gene_info, protein_g, model_metabolites)
...
def secrete_protein(gene_info, modified_protein_g: Protein, model_metabolites) -> ProteinExpressionType:
...
#     if statement1 or lysosomal_degradation_ptm_condition:
    ...
    for secreted_protein in secreted_proteins:
        ...
        # gtph hydrolysis
        rxn[model_metabolites.ntp_map_c['G']], rxn[model_metabolites.h2o_c], rxn[model_metabolites.ndp_map_c['G']], rxn[model_metabolites.pi_c], rxn[model_metabolites.h_c] = -44, -44, 44, 44, 44
...
def get_protein_expression_reactions(gene_info, mrna_transcript_c, mrna_deg_proxy, ub_args: Dict[str, Any], 
                                    modified_trna_transcript_c, charged_trna_map, model_metabolites) -> ProteinExpressionType:
    ...
    if 'Cytosolic Tranport' in gene_info.all_locations.values() or gene_info.L_protein <= params.PTT_LENGTH:
        ...
        if 'Cytosolic Tranport' in gene_info.all_locations.values():
            if len({'c', 'x', 'n'}.intersection(gene_info.all_locations.keys())) > 0: 
                protein_folding_cytosolic, folded_protein_c = fold_protein_cytosolic(gene_info, unfolded_protein_c, model_metabolites=model_metabolites)
                protein_folding_cytosolic._final_compartments = protein_folding_cytosolic._final_compartments.union(set(gene_info.all_locations).intersection({'c', 'x', 'n'}))
                ...
                if 'c' in gene_info.all_locations or 'x' in gene_info.all_locations or ('n' in gene_info.all_locations and folded_protein_c.formula_weight / 1000 <= params.NUCLEAR_DIFFUSION_LIMIT):
                   # cytoplasmic degradation of folded proteins: cytoplasmic proteins, peroxisomal proteins, or nuclear proteins undergoing passive diffusion
                    ...
                    fc = set(gene_info.all_locations).intersection({'c', 'x', 'n'})
                    for r in dr:
                ...
                if 'n' in gene_info.all_locations:
                    nuclear_reactions, folded_protein_n = get_nuclear_reactions(gene_info, folded_protein_c,
                                                                                ub_args=ub_args, model_metabolites=model_metabolites)
                    for r in nuclear_reactions:
                        r._final_compartments.add('n')
    ...
    if 'Canonical Secretion' in gene_info.all_locations.values():
        ...
        if not ptt_:  # co translational translocation
            ctt_reactions, folded_protein_r, gene_info = co_translational_translocation(gene_info, mrna_transcript_c, mrna_deg_proxy, 
                                                                                        modified_trna_transcript_c, charged_trna_map, model_metabolites=model_metabolites)
            for r in ctt_reactions:
                r._final_compartments  = r._final_compartments.union(fc)
            protein_expression_reactions += ctt_reactions
        else:  # post translational translocation
            ptt_reactions, folded_protein_r = post_translational_translocation(gene_info, unfolded_protein_c, model_metabolites=model_metabolites)
        ...
        if 'dsb' in gene_info.ptms or 'gpi' in gene_info.ptms or 'ng' in gene_info.ptms:
            modification_er_reactions, modified_protein_r = modify_protein_er(gene_info, folded_protein_r, model_metabolites=model_metabolites)
...
#         lysosomal_degradation_ptm_condition = 'gpi' in gene_info.ptms.keys() and len(gene_info.ptms.keys()) == 1
        if len(set(['g', 'pm', 'e', 'l']).intersection(gene_info.all_locations.keys())) > 0 or 'og' in gene_info.ptms:  # or lysosomal_degradation_ptm_condition:
            ...
            if 'og' in gene_info.ptms.keys():
                modification_golgi_reactions, modified_protein_g = modify_protein_golgi(gene_info, protein_g, model_metabolites)
            ...
            if len(set(['pm', 'e', 'l']).intersection(gene_info.all_locations.keys())) > 0:  # or lysosomal_degradation_ptm_condition:
                secreted_protein_reactions, secreted_proteins = secrete_protein(gene_info, modified_protein_g, model_metabolites)
                fc = set(gene_info.all_locations).intersection({'pm', 'e', 'l'})
        ...
        if 'l' in gene_info.all_locations or 'pm' in gene_info.all_locations:
            ...
            if 'pm' in gene_info.all_locations:
                ...
                endocytosis_reactions, protein_l = degradation.build_endocytosis_reactions(macromolecule_pm=protein_pm, model_metabolites=model_metabolites,
                                                                                           **{'ub_args': ub_args, 'macromolecule_l': protein_l})
                for r in endocytosis_reactions:
                    r._final_compartments.add('pm')

                ppdr += endocytosis_reactions

            # lysosomal degradation
            fc = set(gene_info.all_locations).intersection({'l', 'pm'})
