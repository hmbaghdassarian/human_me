...
class TrnaInformation:
    def __init__(self, model_metabolites, maturetrna_sequence: str, id_: str, three_trailer_seq: Optional[str] = None, five_leader_seq: Optional[str] = None,
        ...
        #         if anticodon_sequence is not None:
        ...
        if self.five_leader_seq is not None:
            self.pretrna_sequence = self.five_leader_seq + self.maturetrna_sequence[:-3]
        else:
            self.pretrna_sequence = self.maturetrna_sequence[:-3]

        if self.intron_sequences is not None:  # position doesn't matter, just need # of elements and mass balance
            self.pretrna_sequence += ''.join(self.intron_sequences)

        if self.three_trailer_seq is not None:
...
class ExpressTrna:
    ...
    def process_trna(self):
        ...
        rxn[self.trna_info.model_metabolites.ntp_map_n['A']] = -1
        rxn[self.trna_info.model_metabolites.ppi_n] = 3
        trna_processing_machinery = mach.TRNT1.copy()

        # initialize
        # reactions = list()
        rxn[self.trna_info.model_metabolites.h2o_n] = 0
        ...
        if self.trna_info.five_leader_seq is not None:  # if there is a 5' leader sequence
            ...
            rxn[five_leader] = 1
            rxn[self.trna_info.model_metabolites.h2o_n] -= 1  # endonuclolytic cleavage (RNAse P)
            trna_processing_machinery += mach.RNASEP

            five_leader_degradation = five_leader.exonucleolytic_degradation(
                reaction_name=self.trna_info.id + "_5'_leader_fragment_tRNA",
                update=True)
        ...
        if self.trna_info.three_trailer_seq is not None:
            ...
            rxn[three_trailer] = 1
            rxn[self.trna_info.model_metabolites.h2o_n] -= 1  # endonuclolytic cleavage (RNase Z)
            trna_processing_machinery += mach.RNASEZ

            three_trailer_degradation = three_trailer.exonucleolytic_degradation(
                reaction_name=self.trna_info.id + "_3'_trailer_fragment_tRNA",
                update=True)
            self.reactions.append(three_trailer_degradation)

        # splicing of intron
        if self.trna_info.intron_sequences is not None:
            ...
            for i, intron_seq in enumerate(self.trna_info.intron_sequences):
                ...
                rxn[trna_intron_n] = 1
                intron_degradation = trna_intron_n.exonucleolytic_degradation(
                    reaction_name=self.trna_info.id + "_intron_" + str(i) + '_tRNA',
                    update=True)
                self.reactions.append(intron_degradation)

            rxn[self.trna_info.model_metabolites.h2o_n] -= n_introns
    ...
    def primary_export_trna(self):
        ...
        trna_primary_export = ExpressionReaction(self.trna_info.id + '_PRIMARY_EXPORTtn', subsystem='tRNA_Biogenesis')
        ...
        export_rxn[self.trna_info.model_metabolites.ntp_map_c['G']], export_rxn[self.trna_info.model_metabolites.h2o_c], export_rxn[self.trna_info.model_metabolites.ndp_map_c['G']], export_rxn[
            self.trna_info.model_metabolites.pi_c], export_rxn[self.trna_info.model_metabolites.h_c] = -1, -1, 1, 1, 1
    ...
    def charge_trna(self):
        ...
        for code, aa in self.trna_info.model_metabolites.seq_amino_acid_map_c.items():
            ...
            trna_charging = ExpressionReaction('CHARGING_TRNA_' + self.trna_info.id + '_' + code,
                                               ...
                                               trna_charging=True)
            rxn = {self.modified_trna_c: -1, aa: -1, charged_trna_c: 1, self.trna_info.model_metabolites.atp_c: -1, self.trna_info.model_metabolites.ppi_c: 1,
                   self.trna_info.model_metabolites.amp_c: 1}
...
L_mature = trna_data.Mature_Length.value_counts()[
    trna_data.Mature_Length.value_counts() == trna_data.Mature_Length.value_counts().max()].index.tolist()[0]
L_leader = trna_data[' leader length'].value_counts()[
    trna_data[' leader length'].value_counts() == trna_data[' leader length'].value_counts().max()].index.tolist()[0]
L_trailer = trna_data[' trailer length'].value_counts()[
    trna_data[' trailer length'].value_counts() == trna_data[' trailer length'].value_counts().max()].index.tolist()[0]
