...
def get_all_expression_reactions(hgnc_id, reactions, ub_args, psim=params.psim_me,
                                 machinery_list=mach.metabolic_machinery,
                                 compress_mrna=False, nonmachinery_locations: list = None, stochastic=False, seed=None):
    ...
    with warnings.catch_warnings():
        ...
        with func.HiddenPrints():
            gene_info = gene_information.generate_from_psim(hgnc_id, psim, machinery_list, reactions=reactions,
...
def parse_complex_degradation_reaction_id(r_id):
    ...
    if r_id.count('_COMPLEX_') == 1:
        ...
        return '_'.join(
            r_id.split('_')[[i for i in range(len(r_id.split('_'))) if r_id.split('_')[i] == 'COMPLEX'][-1] + 1:])
