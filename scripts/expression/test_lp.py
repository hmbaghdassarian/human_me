...
NMPs = cobra.Metabolite('NMPs')
mrna_deg_proxy = cobra.Metabolite('mrna_deg_proxy')
aas = cobra.Metabolite('aas')
protein = cobra.Metabolite('protein')
mrna = cobra.Metabolite('mrna_c')


# In[7]:


rxn_1 = {mrna: -1, NMPs: 1, mrna_deg_proxy: 1}
r = cobra.Reaction('')
r.add_metabolites(rxn_1)
r.reaction
...
print('c_1rand mrna_c + c_2rand mrna_deg_proxy + amino_acids --> protein')
...
res = pd.read_csv(lp_path + 'optimization_outputs.tab', sep = '\t')
res = res[['mu', 'Algorithm', 'Run_Time', 'Closed_Biomass', 'Status']].sort_values(by = ['mu', 'Closed_Biomass'])
res_quad = res[res.Algorithm == 'QMINOS_quad']
res_double = res[res.Algorithm == 'QMINOS_double']
